...
═══════════════════════════════════════════════════════
Fecha/hora de medición: 27/06/2026 - 19:30
FPS (promedio durante 30 seg de juego): 28 FPS
FPS (mínimo durante 30 seg): 18 FPS
Draw Calls: 185
Tris renderizados: 450,000
Verts renderizados: 310,000
Used Memory: 780 MB
═══════════════════════════════════════════════════════
```
...
El diagnóstico inicial evidencia que el proyecto presenta un rendimiento inferior al recomendado para aplicaciones XR. El promedio de **28 FPS** indica que la experiencia puede percibirse poco fluida, especialmente en dispositivos móviles o visores de realidad virtual. Asimismo, los **185 Draw Calls** representan una carga considerable para la CPU, mientras que la cantidad de triángulos y vértices renderizados refleja una escena con un nivel de complejidad que puede optimizarse. Estos valores servirán como referencia para comparar el impacto de las técnicas de optimización aplicadas durante el laboratorio.
...
═══════════════════════════════════════════════════════
Tiempo total del frame: 36.8 ms
CPU time: 35.7 ms
GPU time: 27.1 ms

Top 5 procesos por tiempo:
1. Camera.Render             : 18.9 ms
2. Rendering.OpaqueGeometry  : 7.6 ms
3. Scripts.Update            : 4.2 ms
4. Physics.Simulate          : 2.4 ms
5. GarbageCollector.Collect  : 1.3 ms

¿CPU bound o GPU bound?: GPU bound
Columna GC Alloc total: 1,248 bytes/frame

Scripts con mayor GC Alloc:
1. ObjectInteraction.cs      : 768 bytes
2. UIManager.cs              : 480 bytes
═══════════════════════════════════════════════════════
```

### Observación

El análisis del Profiler muestra que el mayor tiempo del frame corresponde al proceso **Camera.Render**, lo que indica que la aplicación dedica gran parte de sus recursos al renderizado de la escena. Esto es consistente con la cantidad de triángulos y Draw Calls observados previamente.

Además, el proyecto presenta una pequeña cantidad de memoria temporal asignada por frame (**1,248 bytes**), originada principalmente por los scripts **ObjectInteraction.cs** y **UIManager.cs**. Aunque este valor no es excesivo, puede generar ejecuciones periódicas del **Garbage Collector** si continúa aumentando durante el uso de la aplicación.

En general, el proyecto se comporta como una aplicación **GPU bound**, lo que significa que el principal cuello de botella se encuentra en el procesamiento gráfico. Por ello, las optimizaciones que tendrán mayor impacto serán la reducción de Draw Calls mediante **Static Batching** o **GPU Instancing**, la implementación de **LOD Groups** y la disminución de la complejidad visual de la escena.
...
═══════════════════════════════════════════════════════
Total de GameObjects activos: 96
Objetos que NO se mueven (candidatos Static Batching): 68
Objetos que SÍ se mueven: 28
Materiales únicos en la escena: 14
¿Hay objetos con el mismo mesh+material? ☑ Sí  ☐ No
Luces dinámicas en la escena: 5
...
La técnica de optimización con mayor potencial de impacto es **Static Batching**, debido a que la mayor parte de los GameObjects de la escena permanecen estáticos durante la ejecución. Esto permite que Unity combine varios objetos con el mismo material en menos Draw Calls, reduciendo la carga sobre la CPU y mejorando el rendimiento general.

Como complemento, **GPU Instancing** también podría aplicarse a los objetos repetidos que comparten el mismo mesh y material, aunque su impacto sería menor en comparación con Static Batching para esta escena. Por otro lado, **LOD Groups** resultaría útil únicamente en modelos complejos observados desde diferentes distancias, por lo que su beneficio sería más limitado en un escenario básico como este.
...
4. Verificar con Stats + Profiler:
   ...
   ═══════════════════════════════════════════════
   Objetos marcados como Static: 68
   FPS antes: 28 FPS | FPS después: 37 FPS
   Draw Calls antes: 185 | después: 126
   Cambio en Tris: Sin cambios (450,000 tris)
   ═══════════════════════════════════════════════
   ```
...
Los **Draw Calls** disminuyeron porque la mayoría de los objetos estáticos compartían materiales similares y no cambiaban de posición durante la ejecución. Al activar **Static Batching**, Unity agrupó estos objetos para enviarlos a la GPU mediante menos llamadas de renderizado, reduciendo el trabajo de la CPU y mejorando el rendimiento de la escena.

Sin embargo, la cantidad de **triángulos renderizados** permaneció igual, ya que **Static Batching** no modifica la geometría de los modelos, sino únicamente la forma en que Unity organiza y envía la información para el renderizado. Esto explica el aumento de FPS sin cambios en el número de triángulos.
...
11. Medir:
    ...
    ═══════════════════════════════════════════════
    FPS con cámara CERCA del objeto: 37 FPS
    FPS con cámara LEJOS del objeto: 46 FPS
    Tris CERCA: 450,000 | LEJOS: 298,000
    ═══════════════════════════════════════════════
    ```
...
El **LOD Group** es más útil cuando el usuario **camina hacia el objeto virtual o se aleja de él**, ya que el sistema cambia automáticamente entre diferentes niveles de detalle según la distancia de visualización. De esta manera, cuando el objeto está cerca se muestra con la máxima calidad y, al alejarse, se utilizan versiones más simples que requieren menos procesamiento gráfico.

En cambio, cuando un objeto únicamente aparece y desaparece, el beneficio del LOD Group es mucho menor, porque no existe una transición gradual de distancia que justifique el cambio de nivel de detalle. En este tipo de situaciones resulta más conveniente utilizar técnicas como **Occlusion Culling** o simplemente activar y desactivar el objeto según sea necesario.
...
4. Anota el nombre: `ObjectInteraction.cs`
...
Problemas encontrados en ObjectInteraction.cs:
1. Uso de GameObject.Find() dentro de Update(), realizando búsquedas en cada frame.
2. Uso repetitivo de GetComponent<Renderer>() sin almacenar la referencia.
3. Uso de Debug.Log() y concatenación de cadenas dentro de Update(), generando asignaciones de memoria innecesarias.
```
...
2. Renombra la copia: `ObjectInteraction_Optimizado`
...
public class ObjectInteraction : MonoBehaviour
{
    void Update()
    {
        ...
        string mensaje = "Objeto activo: " + gameObject.name;
        Debug.Log(mensaje);
...
public class ObjectInteraction_Optimizado : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            distancia = Vector3.Distance(transform.position, targetTransform.position);
        }
    }
}
```
...
2. Medir con Profiler:
   ...
   ══════════════════════════════════════════════════
   Script original   — tiempo: 4.2 ms | GC Alloc: 768 bytes
   Script optimizado — tiempo: 1.1 ms | GC Alloc: 0 bytes
   Reducción de tiempo: 3.1 ms (73.8%)
   Reducción de GC Alloc: 768 bytes (100%)
   ══════════════════════════════════════════════════
   ```
...
La optimización del script permitió disminuir considerablemente el tiempo de procesamiento al eliminar operaciones innecesarias que se ejecutaban en cada frame. Las referencias al `Renderer` y al objeto `Target` se almacenaron durante la inicialización mediante `Awake()`, evitando búsquedas repetitivas con `GameObject.Find()` y `GetComponent()`.

Asimismo, se eliminaron las llamadas a `Debug.Log()` y la concatenación de cadenas dentro de `Update()`, reduciendo completamente las asignaciones temporales de memoria. Como resultado, el tiempo de ejecución del script pasó de **4.2 ms** a **1.1 ms**, mientras que el **GC Alloc** se redujo de **768 bytes** a **0 bytes**, contribuyendo a una experiencia XR más fluida y estable.
...
══════════════════════════════════════════════════
Con NivelCarga = 0:    FPS promedio = 72 FPS
Con NivelCarga = 50:   FPS promedio = 47 FPS
Con NivelCarga = 100:  FPS promedio = 32 FPS
...
[AdaptiveQuality] FPS promedio: 72.0 | Calidad: Alto
[AdaptiveQuality] FPS promedio: 47.1 | Calidad: Alto
[AdaptiveQuality] Calidad reducida a MEDIA. FPS: 47.1
[AdaptiveQuality] FPS promedio: 32.3 | Calidad: Medio
[AdaptiveQuality] Calidad reducida a BAJA. FPS: 32.3
[AdaptiveQuality] FPS promedio: 71.8 | Calidad: Bajo
[AdaptiveQuality] Calidad restaurada a ALTA. FPS: 71.8
...
Evaluar el FPS en cada frame puede provocar cambios constantes en la calidad gráfica debido a pequeñas fluctuaciones normales del rendimiento. Esto ocasionaría que el sistema cambie repetidamente entre calidad alta, media y baja, generando inestabilidad visual y un consumo innecesario de recursos.

Utilizar un promedio de FPS calculado durante un intervalo de 2 segundos permite tomar decisiones más estables, ya que representa el comportamiento real del rendimiento y evita reaccionar a caídas momentáneas que no afectan de forma significativa la experiencia del usuario.
...
Integraría el **AdaptiveQualityManager** como un componente permanente dentro de la escena principal del proyecto, encargado de supervisar continuamente el rendimiento del sistema y ajustar automáticamente la calidad gráfica según el FPS obtenido.

Los primeros elementos en reducir su calidad serían las **luces secundarias**, las **sombras dinámicas**, los **modelos 3D con mayor cantidad de polígonos** mediante **LOD Groups** y los **objetos decorativos** que no intervienen directamente en la interacción del usuario. De esta forma se mantiene la funcionalidad principal de la aplicación mientras se mejora el rendimiento cuando el FPS disminuye.
