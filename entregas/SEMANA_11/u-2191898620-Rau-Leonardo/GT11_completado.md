...
| Campo | Detalle |
|-------|---------|
| **Estudiante** | _Rau Bravo Leonardo Cesar_ |
| **Código** | _2191898620_ |
| **Fecha** | _30/06/2026_ |
| **Semana** | 11 — Scripts e Inputs en XR |
...
|-----------|-------------|-----------|
| 1. `TouchPhase.Began` | _C_ | A. Evento que se dispara cuando el controller apunta a un interactable |
| 2. `InputActionReference` | _D_ | B. Lee el valor de una acción en cada frame (para entradas continuas) |
| 3. `EnhancedTouchSupport.Enable()` | _E_ | C. Solo el primer frame en que el dedo toca la pantalla |
| 4. `ReadValue<T>()` en Update | _B_ | D. Campo público que une el script con una acción del Input Actions Asset |
| 5. `hoverEntered` | _A_ | E. Activa el API avanzado de toques multi-dedo del nuevo Input System |
| 6. `performed` | _F_ | F. Callback que se dispara cuando se activa una InputAction |
| 7. `XR Device Simulator` | _H_ | G. Limpiar la lista en vez de crear una nueva con `new` |
| 8. `hits.Clear()` | _G_ | H. Herramienta para probar XR con teclado y ratón en el editor |
| 9. `cancelado` / `canceled` | _I_ | I. Callback que se dispara cuando se desactiva/suelta una InputAction |
| 10. `selectEntered` | _J_ | J. Evento de XRGrabInteractable cuando el usuario agarra el objeto |
